...
const TaskResponseTypeTable = ({ typeHeaders, students, tasks, selectType, updatedTasks }) => {
  ...
  return (
    <Table>
      <Table.Header>
        ...
        <Table.Row>
          {updatedHeaders.map(header => header.types.map(type => <Table.HeaderCell key={type.id}><Button onClick={selectType} type={type} color={type.updated ? 'red' : 'grey'}>{type.name}</Button></Table.HeaderCell>))}
        </Table.Row>
      </Table.Header>
      <Table.Body>
        {students.map(person => (
          <Table.Row key={person.id}>
            ...
            {updatedHeaders.map(header => header.types.map(type => (
